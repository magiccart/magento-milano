...
abstract class Magiccart_Blog_Block_Abstract extends Mage_Core_Block_Template
{
    ...
    protected static $_catUriParam = Magiccart_Blog_Helper_Data::CATEGORY_URI_PARAM;

    protected static $_postUriParam = Magiccart_Blog_Helper_Data::POST_URI_PARAM;

    protected static $_tagUriParam = Magiccart_Blog_Helper_Data::TAG_URI_PARAM;
    ...
    protected function _processCollection($collection, $category = false)
    {
        ...
        foreach ($collection as $item) {
            ...
            if ($shortContent) {
                ...
                $strManager = new Magiccart_Blog_Helper_Substring(
                    array(
    ...
    {
        if (self::$_helper->conf(Magiccart_Blog_Helper_Data::XML_BOOKMARKS)) {
            return $this->setTemplate('magiccart/blog/bookmark.phtml')->setPost($post)->renderView();
        }
    }
    ...
    public function getTagsHtml($post)
    {
        if (trim($post->getTags())) {
            return $this->setTemplate('magiccart/blog/line_tags.phtml')->setPost($post)->renderView();
        }
    }
    ...
    {
        return Mage::app()->getRequest()->getModuleName() == Magiccart_Blog_Helper_Data::DEFAULT_ROOT;
    }
